...
# 2. Importing the database plainsheet from the Electricity Information Disclosure
# db_raw <- readRDS(file = "~/Workplace/db_raw.rds")

## 2.1. Importing Dataset from ComCom archive 
...
## 2.2 Renaming column to remove spaces
...
```{r load-map,echo=FALSE,cache=TRUE}
       ## 2.3. Importing the Polygons of New Zealand Regions
nz_region <- geojsonio::geojson_read("~/Workplace/01-Maps/nz_region.geojson",
                                      what = "sp")

       ## 2.4. Importing the Polygons of New Zealand Territories
nz_territories <- geojsonio::geojson_read("~/Workplace/01-Maps/nz_territories.geojson",
                                      what = "sp")
```

```{r data-manipulation,echo=FALSE,include=FALSE,cache=FALSE}
# 3. Creating a Neaty Data ####

## 3.1. Removing unuseful columns.
db_raw <- db_raw %>% select(everything(),
                            -Disclosure_year,
                            -Schedule,
                            -Schedule_reference,
                            -Forecast_year,
...
## 3.2. Ordering the raw data
db_raw <- db_raw %>% arrange(-Year,EDB,Network)

## 3.3 Removing the space between column names
...
Unfortunately, the dataset provided by the New Zealand Commerce Commission (NZCC) is not a [*tidy dataset*](http://garrettgman.github.io/tidying/) according to [@Grolemund], and for this reason, I have created a new version of the database so-called neat database.
...
In this chapter, I will show a straightforward statistical descriptive without the intention of deep understanding and what drive the results. I only want to know the big picture of the company, what is the trend and the overall rates. For this reason, this chapter is very long and exhaustively. You may want to waived this chapter to the [Chapter 3](#chapter3), which is richier in analysis.
...
<center>
<img title="Image extracted from 2018 Annual Report" src="05-Figure/ae_map.svg" alt="Aurora Energy Area" width="320" height="320">
</center>


###### New Zealand Map

```{r map-nz,echo=FALSE,cache=TRUE,out.width="100%",fig.height=3}
...
<iframe width="90%" height="400" src="https://www.mindomo.com/mindmap/aurora-energy-df2907a8292742ee8f7e1c192781d447" frameborder="0" allowfullscreen>Your browser does not support frames. <a href="https://www.mindomo.com/mindmap/aurora-energy-df2907a8292742ee8f7e1c192781d447" target="_blank">View</a> this map on its original site. It was created using <a href="https://www.mindomo.com" target="_blank">Mindomo</a>.</iframe>
...
* Aurora Energy does not have various non-standard consumers as shown in Table 70.1. In 2018 the share in Total Energy Delivered to this load-group was only `r round(100*(10563/1308293),2)`%.
...
```{r echo=FALSE}
# The db_consumer_group is a subset of db_raw, only has the number of consumer
db_consumer_group %>%
       ...
       filter(Section %in% "8(i): Billed Quantities by Price Component",
              ...
              table71[,3:9] <- round(table71[,3:9],0) # Rounding the numbersof columns 3 to 9

# Printing table as DT.
datatable(data = table71,caption = "Aurora Energy Consumers")
```
...
* Aside from the typos observed in Graphic 72.1, it is possible to see a positive trend in Central Otago and a slight positive trend in Dunedin. Aggregating these trends result in a global growth of `r round(((88569/83305)-1)*100,2)`% accumulated in 6 years.
...
<a href="https://rpubs.com/AndersonUyekita/QJ45141_dashboard" target="blank">Aurora Energy Dashboard</a>
...
The aim of the clusterization is to find groups in a population with same similarity, but underlying this main objective resides many details, which will be discussed in the next subchapters. The methodology applied in this chapter is based on the book @Kassambara.
...
```{r echo=FALSE}
# Subsetting the db_neat to create the db_cluster
db_cluster <- db_neat %>% 
       filter(Year %in% 2018,
              Network %in% "All") %>% # Excluding the Regionals
              select(EDB,Year,COD,island,
                     rev_con_tot_ave_co, # 1. Number of ICPs
                     rev_con_tot_mwh,
                     r_saidi,
                     rg_a_com_tot,
                     rg_tax_income,
...
db_cluster <- as.data.frame(db_cluster)

rownames(db_cluster) <- db_cluster$COD



ggcorr(db_cluster[,-c(1:5)],geom = "circle", label = TRUE)
...
\begin{equation}
\small obs_{standard} = \frac{(x_{i} - centre_{i})}{scale(x)}
\end{equation}


$\small
\begin{array}{l l}
     obs_{standard}:   & \text{standardizated observation} \\
     x_{i}:            & \text{observation i} \\
     y_{i}:            & \text{mean of all x observations}\\
     scale(x):         & \text{standard deviation of the x observation}\\
\end{array}$
...
```{r echo=FALSE}
# Standardization
db_cluster_std <- scale(as.matrix(db_cluster[,-c(1:5)]))


PCA(X = db_cluster_std,
    scale.unit = TRUE,
    ncp = 3,
    graph = TRUE)


pca_res <- PCA(X = db_cluster_std,
    scale.unit = TRUE,
    ncp = 3,
    graph = FALSE)
...
\begin{equation}
\small d_{man}(x,y) = \sum_{i=1}^n|(x_{i} - y_{i})|
\end{equation}


$\small
\begin{array}{l l}
     d_{man}:        & \text{Distance between x and y} \\
     x_{i}:          & \text{} \\
     y_{i}:          & \text{} \\
\end{array}$
...
db_cluster_std_dist <- get_dist(db_cluster_std, method = "manhattan")

fviz_dist(db_cluster_std_dist,
          show_labels = TRUE) +
...
\begin{equation}
\small H = \frac{\sum_{i=1}^n{y_{i}}}{\sum_{i=1}^n{x_{i}}+\sum_{i=1}^n{y_{i}}}
\end{equation}
...
```{r echo=FALSE}
# Statistic H
h <- hopkins(db_cluster_std, n = nrow(db_cluster_std)-1)
```

If H is close 0.5 means that there are not clusters.

The results of this statistic in the NZ Electricity Market is `r round(h[[1]],2)`, which means there are clusters in this dataset.
...
```{r echo=FALSE}
# Silhouette Calculationg to Optimized number of cluster
fviz_nbclust(as.matrix(db_cluster_std), pam, method = "silhouette") +
       theme_classic()
```
...
# Gap statistic
set.seed(123)
fviz_nbclust(as.matrix(db_cluster_std), pam, method = "gap_stat", nboot = 500)+
...
```{r echo=FALSE}
pam_res <- pam(as.matrix(db_cluster_std_dist),2)
...
```{r echo=FALSE}
### 4.3.3. PAM Clustering
# Tree creation
hc_res <- hclust(d = dist(db_cluster_std), method = "ward.D2")
```
...
# Visualisation
# cex: label size
fviz_dend(hc_res, cex = 0.5)


fviz_dend(hc_res, k =4, # Cut in four groups
          cex = 0.5, # label size 
          horiz = TRUE,
          k_colors = "jco",
          rect = TRUE,
...
coph_res <- cophenetic(hc_res)

# Correlation between cophenetic distance and
# the original distance
cor(dist(db_cluster_std), coph_res)
```



```{r echo=FALSE}
hc_res2 <- hclust(d = dist(db_cluster_std), method = "average")


coph_res2 <- cophenetic(hc_res2)

cor(dist(db_cluster_std), coph_res2)
...
```{r echo=FALSE}
library(dendextend)
dend_compare <- dendlist(as.dendrogram(hc_res),as.dendrogram(hc_res2))

tanglegram(as.dendrogram(hc_res), as.dendrogram(hc_res2))
...
4. Table 4 - Cabos subterrâneos: 2/3 estão em Central Otago, cujo crescimento é igual em ambas regionais (Central Otago é de `r round(100*(1497-1077)/1077,1)`% e Dunedin é de `r round(100*(778-561)/561,1)`%).
